...
char sh_sem1[] = "/posix-sem1";
char sh_sem2[] = "/posix-sem2";
char sh_sem3[] = "/posix-sem2";
...
int main(int argc, char **argv)
{
    ...
    if (shmid_sem1 = shm_open(sh_sem1, O_CREAT | O_RDWR, 0666) == -1)
    {
        printf("Opening error\n");
    ...
    ftruncate(shmid_sem1, sizeof(sem_t));
    //printf("Here 2\n");

    sem1 = mmap(0, sizeof(sem_t), PROT_WRITE | PROT_READ, MAP_SHARED | MAP_ANON, shmid_sem1, 0);
    //printf("Here 3\n");
    ...
    if (shmid_sem2 = shm_open(sh_sem2, O_CREAT | O_RDWR, 0666) == -1)
    {
        printf("Opening error\n");
        perror("parent shm_open");
        return 1;
    ...
    ftruncate(shmid_sem2, sizeof(sem_t));

    sem2 = mmap(0, sizeof(sem_t), PROT_WRITE | PROT_READ, MAP_SHARED | MAP_ANON, shmid_sem2, 0);
    sem_init(sem2, 1, 0);

    if (shmid_sem3 = shm_open(sh_sem3, O_CREAT | O_RDWR, 0666) == -1)
    {
        printf("Opening error\n");
        perror("parent shm_open");
        return 1;
    ...
    ftruncate(shmid_sem3, sizeof(sem_t));

    sem3 = mmap(0, sizeof(sem_t), PROT_WRITE | PROT_READ, MAP_SHARED | MAP_ANON, shmid_sem3, 0);
    sem_init(sem3, 1, 1);
