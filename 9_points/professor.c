...
int main(int argc, char **argv)
{
    ...
    if ((msqid1 = msgget(10, IPC_CREAT)) < 0) {
        perror("msgget 1");
        exit(-1);
    }

    if ((msqid2 = msgget(20, IPC_CREAT)) < 0) {
        perror("msgget 2");
        exit(-1);
    }
    ...
    while (k > 0)
    {
        ...
        if (msgrcv(msqid1, &message, sizeof(Message), 1, 0) < 0) {
            perror("msgget");
            exit(-1);
        }
        ...
        {
            printf("Professor received a variant from Student:\n      %s\n", message.message);
        }
        else
        {
            printf("Professor received an answer from Student:\n      %s\n", message.message);
