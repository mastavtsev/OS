...
void SendMessageStudent(int i, int message_type)
{
    ...
    if (message_type == 0)
    {
        snprintf(message.message, 256, "Student %d: my variant is %d", i, rand() % 25 + i + 1);
    }
    else
    {
        snprintf(message.message, 256, "Student %d: my answer is ANSWER %d", i, rand() % 25 + i + 1);
    }
...
void ReceiveMessageStudent(int i)
{
    ...
    if (msgrcv(msqid2, &message, 256, 1, 0) < 0) {
        perror("msgget");
        exit(-1);
    }
...
int main(int argc, char **argv)
{
    ...
    if ((msqid1 = msgget(10, 0666)) < 0) {
        perror("msgget 1");
        exit(-1);
    }

    if ((msqid2 = msgget(20, 0666)) < 0) {
        perror("msgget 2");
        exit(-1);
    }
