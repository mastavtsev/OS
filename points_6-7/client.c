...
int send_data(int socket, void *data, size_t size)
{
    ...
    {
        bytes_sent = sendto(socket, buffer + total_bytes_sent, size - total_bytes_sent, 0, (struct sockaddr *)&serverAddr, sizeof(serverAddr));
        if (bytes_sent < 0)
        {
...
int main(int argc, char **argv)
{
    ...
    if (recvfrom(clientSocket, buffer_recv, BUFFER_SIZE, 0, (struct sockaddr *)&serverAddr, &addr_size) < 0)
    {
