...
char memn[] = "/shared-memory"; // адрес разделяемой памяти логов
...
int main(int argc, char **argv)
{
    ...
    if ((childpid = fork()) > 0)
    {
        ...
        int clients_main_sock = create_soket(clientsPort);
    ...
    {
        int observers_main_sock = create_soket(observerPort);
        ...
        while (1)
        {
            ...
            while (1)
            {
                ...
                if (send_data(observer_socket, addr, BUFFER_SIZE) < 0)
                {
